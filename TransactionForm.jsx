...
import { CATEGORIES, findDuplicate, currentBalance, formatMoney } from './helpers';
...
export default function TransactionForm({ transactions, prefill, onPrefillConsumed }) {
  ...
  async function handleSubmit(e) {
    ...
    // Balance can never go negative: an expense can never exceed the current balance.
    if (type === 'expense') {
      const balance = currentBalance(transactions || []);
      if (Number(amount) > balance) {
        setError(
          `This expense exceeds your current balance of ${formatMoney(balance)}. Add income first, or reduce the amount.`
        );
        return;
      }
    }
